...
export type Support = {
  ...
  ranges: Record<
    string,
    { min: number; max: number; p01: number; p99: number }
  >;
...
export function validateScenario(value: unknown): string[] {
  ...
  if (
    ...
    errors.push('Date must be a valid YYYY-MM-DD calendar date.');
  if (!Number.isInteger(s.hour) || s.hour < 0 || s.hour > 23)
  ...
  for (const key of [
    ...
    if (s[key] < 0) errors.push(`${key} cannot be negative.`);
  if (s.humidity < 0 || s.humidity > 100)
    errors.push('Humidity must be between 0 and 100%.');
  if (!seasons.includes(s.season))
    errors.push('Season must be Winter, Spring, Summer or Autumn.');
  if (typeof s.holiday !== 'boolean' || typeof s.functioning !== 'boolean')
    errors.push('Holiday and functioning must be booleans.');
  return errors;
}
export function expandFeatures(s: Scenario): number[] {
  ...
  return [
    ...
    Number(s.holiday),
    Number(weekday >= 5),
    Math.sin((s.hour * twoPi) / 24),
    Math.cos((s.hour * twoPi) / 24),
    Math.sin((dayIndex * twoPi) / 365.25),
    Math.cos((dayIndex * twoPi) / 365.25),
...
export function predict(model: Model, scenario: Scenario): number {
  ...
  if (x.length !== model.features.length)
    throw new Error('Model feature schema does not match inference.');
...
export function scenarioWarnings(
  ...
  if (!s.functioning)
    warnings.push(
      'Service closed: new rentals are set to zero by an operating rule. This does not measure unmet demand.',
    );
  if (seasonForDate(s.date) !== s.season)
    warnings.push('Experimental calendar: selected season and date disagree.');
  if (s.date < '2017-12-01' || s.date > '2018-11-30')
    warnings.push(
      'This date is outside the source observation year. Current travel patterns are not represented.',
    );
  const outside = Object.entries(support.ranges)
    .filter(
      ([k, range]) =>
        (s[k as keyof Scenario] as number) < range.min ||
        (s[k as keyof Scenario] as number) > range.max,
    )
    .map(([k]) => k);
  if (outside.length)
    warnings.push(`Outside training range: ${outside.join(', ')}.`);
  ...
  for (const row of support.referenceVectors) {
    ...
    for (let j = 0; j < raw.length; j++)
      distanceSquared += ((raw[j] - row[j]) / support.scale[j]) ** 2;
    if (distanceSquared < nearestSquared) nearestSquared = distanceSquared;
  }
  if (Math.sqrt(nearestSquared) > support.threshold)
    warnings.push(
      'Unusual combination: few operating training examples resemble these conditions. Interpret this estimate cautiously.',
    );
  return warnings;
}
export type LocalContribution = {
  label: string;
  fields: (keyof Scenario)[];
...
export function explainChange(
  ...
  for (const group of groups) {
    ...
    const from = Object.fromEntries(
      group.fields.map((key) => [key, intermediate[key]]),
    );
    const to = Object.fromEntries(
      group.fields.map((key) => [key, scenario[key]]),
    );
  ...
  return {
    method:
      'Ordered input replacement: calendar, hour, temperature, humidity, wind, visibility, dew point, solar, rain, snow, availability. Contributions depend on this order and intermediate combinations. They describe the model, not causal effects.',
    baselinePrediction,
    prediction,
    delta: prediction - baselinePrediction,
    percentChange:
      baselinePrediction === 0
        ? null
        : (100 * (prediction - baselinePrediction)) / baselinePrediction,
    contributions,
  };
}
